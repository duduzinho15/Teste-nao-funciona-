...
CREATE TABLE public.links_para_coleta (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT ck_links_status CHECK (((status)::text = ANY ((ARRAY['pendente'::character varying, 'processando'::character varying, 'concluido'::character varying, 'erro'::character varying])::text[])))
);
...
CREATE TABLE public.partidas (
    ...
    CONSTRAINT ck_partidas_status CHECK (((status)::text = ANY ((ARRAY['agendada'::character varying, 'em_andamento'::character varying, 'finalizada'::character varying, 'cancelada'::character varying])::text[])))
);
