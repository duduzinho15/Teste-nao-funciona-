...
class EstatisticasJogador:
    ...
    jogador_nome: str
    time_nome: str
    nacao: str = field(default_factory=str)
    posicao: str = field(default_factory=str)
    idade: str = field(default_factory=str)
...
class ColetorEstatisticas:
    ...
    def __init__(self, db_path: str = DB_NAME, session: Optional[Session] = None) -> None:
        ...
        self.advanced_scraper: Optional[AdvancedMatchScraper] = (
            AdvancedMatchScraper(session) if session else None
        )
        
        # Mapeamento de data-stat para campos da classe
        self.stat_mapping: Dict[str, Tuple[str, type]] = self._create_stat_mapping()

    def _create_stat_mapping(self) -> Dict[str, Tuple[str, type]]:
        """Cria mapeamento entre data-stat HTML e campos da classe.
        
        Returns:
            Dict[str, Tuple[str, type]]: Mapeamento de campos HTML para atributos da classe
        ...
        return {
            ...
            # Estatísticas de gols e finalização
            'goals': ('gols', int),
            'assists': ('assistencias', int),
            'pens_made': ('gols_penalti', int),
            'pens_att': ('penaltis_cobrados', int),
            'shots_total': ('finalizacoes', int),
            'shots_on_target': ('finalizacoes_no_gol', int),
            'shots_on_target_pct': ('precisao_finalizacao', float),
            
            # Cartões
            'cards_yellow': ('cartoes_amarelos', int),
            'cards_red': ('cartoes_vermelhos', int),
            'cards_yellow_red': ('amarelos_que_viraram_vermelho', int),
            ...
            'xa': ('xg_assist_jogador', float),
            'npxg_xa': ('xg_npxg_assist_jogador', float),
            'progressive_passes': ('passes_progressivos', int),
            'progressive_carries': ('dribles_progressivos', int),
            'touches': ('toques', int),
            'pressures': ('pressoes', int),
            'tackles': ('desarmes', int),
            'interceptions': ('interceptacoes', int),
            'blocks': ('bloqueios', int),
            'clearances': ('afastamentos', int),
            'aerials_won': ('desafios_aereos_ganhos', int),
    ...
    def get_db_connection(self) -> Generator[sqlite3.Connection, None, None]:
        ...
        Raises:
            sqlite3.Error: Em caso de erro na conexão com o banco de dados
        """
    ...
    def extrair_stat_seguro(
        self, 
        linha: Tag, 
        stat_name: str, 
        tipo: type = float
    ) -> Union[int, float, str]:
        """Extrai e converte uma estatística de forma segura.
        
        Args:
            linha: Elemento HTML da linha contendo a estatística
            stat_name: Nome do atributo data-stat a ser extraído
            tipo: Tipo para conversão (int, float, str). Padrão: float
            
        Returns:
            Valor convertido para o tipo especificado ou valor padrão do tipo em caso de erro
            
        Examples:
            >>> # Extrai um número inteiro de uma tag HTML
            >>> valor = extrair_stat_seguro(linha, 'goals', int)
            3
        """
        valor_padrao: Union[int, float, str] = tipo()
        
        try:
            # Encontra o elemento com o data-stat especificado
            elemento = linha.find(attrs={"data-stat": stat_name})
    ...
    def extrair_estatisticas_jogador(self, linha_sum: Tag, linha_pass: Tag, linha_pass_types: Tag,
                                   linha_def: Tag, linha_poss: Tag, linha_misc: Tag,
    ...
    def processar_match_report(self, soup: BeautifulSoup, partida_id: int, match_url: Optional[str] = None) -> Tuple[bool, int]:
        """
