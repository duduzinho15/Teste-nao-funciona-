...
class SportspageFeedsAPI(RapidAPIBase):
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        config = RapidAPIConfig(
            ...
            base_url="https://sportspage-feeds.p.rapidapi.com",
            api_key=api_key,
    ...
    async def coletar_jogos(self, esporte: Optional[str] = None,
        ...
        """
        Coleta jogos dos feeds do Sportspage.
        ...
        try:
            ...
            response = await self._make_request(
                endpoint=self.endpoints['resultados'],
                params=params
            )
            
            if not response or 'games' not in response:
                logger.warning("Resposta da API não contém dados de jogos")
                return []
            
            jogos = []
            for jogo in response['games']:
                jogo_padronizado = self._padronizar_jogo(jogo)
                if jogo_padronizado:
                    jogos.append(jogo_padronizado)
            
            logger.info(f"Coletados {len(jogos)} jogos do Sportspage Feeds")
            return jogos
    ...
    async def coletar_noticias(self, esporte: Optional[str] = None,
                              categoria: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        Coleta notícias dos feeds do Sportspage.
        ...
        try:
            ...
            response = await self._make_request(
                endpoint=self.endpoints['noticias'],
                params=params
            )
            
            if not response or 'news' not in response:
                logger.warning("Resposta da API não contém dados de notícias")
                return []
            
            noticias = []
            for noticia in response['news']:
                noticia_padronizada = self._padronizar_noticia(noticia)
                if noticia_padronizada:
                    noticias.append(noticia_padronizada)
            
            logger.info(f"Coletadas {len(noticias)} notícias do Sportspage Feeds")
            return noticias
