...
class FootballProAPI(RapidAPIBase):
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        config = RapidAPIConfig(
            ...
            base_url="https://football-pro.p.rapidapi.com",
            api_key=api_key,
    ...
    async def coletar_analises_avancadas(self, partida_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        Coleta análises avançadas da API Football Pro.
        
        Args:
            partida_id: ID da partida (opcional)
            
        Returns:
            Lista de análises avançadas padronizadas
        """
        try:
            params = {}
            if partida_id:
            ...
            response = await self._make_request(
                endpoint=self.endpoints['analises'],
                params=params
            )
            
            if not response or 'analysis' not in response:
                logger.warning("Resposta da API não contém dados de análises")
                return []
            
            analises = []
            for analise in response['analysis']:
                analise_padronizada = self._padronizar_analise_avancada(analise)
                if analise_padronizada:
                    analises.append(analise_padronizada)
            
            logger.info(f"Coletadas {len(analises)} análises avançadas da Football Pro")
            return analises
            
        except Exception as e:
            logger.error(f"Erro ao coletar análises avançadas: {e}")
            return []
    
    async def coletar_insights_taticos(self, partida_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        Coleta insights táticos da API Football Pro.
        
        Args:
            partida_id: ID da partida (opcional)
            
        Returns:
            Lista de insights táticos padronizados
        """
        try:
            params = {}
            if partida_id:
            ...
            response = await self._make_request(
                endpoint=self.endpoints['insights'],
                params=params
            )
            
            if not response or 'insights' not in response:
                logger.warning("Resposta da API não contém dados de insights")
                return []
            
            insights = []
            for insight in response['insights']:
                insight_padronizado = self._padronizar_insight_tatico(insight)
                if insight_padronizado:
                    insights.append(insight_padronizado)
            
            logger.info(f"Coletados {len(insights)} insights táticos da Football Pro")
            return insights
            
        except Exception as e:
            logger.error(f"Erro ao coletar insights táticos: {e}")
            return []
